    ...
    fileInput.onchange = (e) => {
      ...
      if (file) {
        ...
        reader.onload = async (event) => {
          ...
          if (imageSafe === null) {
            setProfileResponse(
              `Error: Service to uplaod image is currently unavailable`
            );
          }

          imageSafe
            ? setProfileResponse(
                'Image added Successfully, please save changes'
              )
            : setProfileResponse(
                `Error: Image uploaded isn't safe, try another`
              );
        ...
        <>
          ...
          {profileResponse ? (
            <div>
              <p
                className={
                  profileResponse.includes('Error')
                    ? 'text-red'
                    : 'text-green-500'
                }
              >
